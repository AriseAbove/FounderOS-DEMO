...
/**
 * Branding: the sidebar reads ARISE OS with the OS
 * ring mark — the mark only, the wordmark rides
...
describe('OS mark branding', () => {
  test('the mark is the ring with an UPRIGHT letter-S seam in brand red', () => {
    ...
    // vertical S (design: "vertical like the O") — stacked arcs, no rotation
  ...
  test('the sidebar brands with the mark, no raster emblem', () => {
    ...
    expect(sidebar).toContain('ARISE OS');
